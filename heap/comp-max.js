...
class ExtMaxHeap {
    ...
    heapify(arr, size, index) {
        ...
        if (left < size && this.compare(arr[left], arr[largest]) > 0) {
            largest = left
        }

        if (right < size && this.compare(arr[right], arr[largest]) > 0) {
            largest = right
        }

        if (largest !== index) {
            this.swap(largest, index, arr);
